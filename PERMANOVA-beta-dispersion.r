...
adonis_full_strata <- adonis2(bray_dist ~ subtype + site + month.year,data = meta,permutations = 999,strata = meta$colony.name,by = "margin")
print(adonis_full_strata)

# -----------------------------
# Beta-dispersion
# -----------------------------
# 1. Dispersion among sample subtypes
...
# 2. Dispersion among sites
bd_site <- betadisper(bray_dist, group = meta$site)
anova(bd_site)
permutest(bd_site, permutations = 999)

# 3. Dispersion among months
bd_month <- betadisper(bray_dist, group = meta$month.year)
anova(bd_month)
permutest(bd_month, permutations = 999)

# -----------------------------
# Pairwise PERMANOVA
# -----------------------------
pairwise_results <- pairwise.adonis2(
  bray_dist ~ subtype + site + month.year,
  data = meta,
